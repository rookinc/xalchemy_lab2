...
# Build graph from Q off-diagonal 9
edges = []
...
# all local edge types tau_x(e) that occur
all_types = set()
edge_types_by_vertex = {x: {} for x in range(n)}
for x in range(n):
    for e in edges:
        u, v = e
        t = tuple(sorted((dist[x][u], dist[x][v])))
        edge_types_by_vertex[x][e] = t
        all_types.add(t)

all_types = sorted(all_types)

print("Edge-types that occur:")
for t in all_types:
    print(" ", t)
print()

def supports_from_type_subset(T):
    """Build row sectors by local type rule, then derive column supports."""
    T = set(T)

    # row sectors: for each vertex x, which actual edges are selected
    row_sectors = []
    for x in range(n):
    ...
    # edge -> set of vertices whose row-sector contains it
    edge_to_vertices = {}
    for e in edges:
...
print("Testing all subsets of edge-types...\n")
...
for r in range(1, len(all_types) + 1):
    for subset in combinations(all_types, r):
        ...
        # only care about rules with constant row size 14
...
if not found:
    print("No exact type-subset rule found.")
