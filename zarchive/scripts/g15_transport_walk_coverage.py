...
def path_for_tick(tick: int) -> dict[str, list[int]]:
...
def coverage_after_passes(num_passes: int) -> dict:
    covered: set[tuple[int, int]] = set()
    per_pass = []

    for tick in range(1, num_passes + 1):
        paths = path_for_tick(tick)
        tick_edges: set[tuple[int, int]] = set()
        region_edges = {}

        for region, path in paths.items():
            region_set = edges_from_path(path)
            region_edges[region] = sorted(region_set)
            tick_edges |= region_set
        ...
        per_pass.append(
            {
                ...
                "tick_edge_count": len(tick_edges),
...
def main():
    ...
    for passes in range(1, 13):
        data = coverage_after_passes(passes)
        summaries.append(
            {
                "passes": passes,
    ...
    out = {
        ...
        "coverage_table": summaries,
        "detail_at_12_passes": coverage_after_passes(12),
    }
